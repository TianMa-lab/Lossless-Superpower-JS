...
class ExtractionEngine {
  ...
  async extract(data, options = {}) {
    ...
    for (const [layer, extractor] of Object.entries(this.extractors)) {
      if (options.layers && !options.layers.includes(layer)) {
        continue;
      }
      result[layer] = await extractor(data, options);
    }

    return result;
  }

  extractRawData(data, options) {
    return {
      original: data,
      timestamp: Date.now(),
  ...
  extractStructuredData(data, options) {
    const structured = {
      entities: [],
      keywords: [],
      relationships: [],
  ...
  extractSemanticData(data, options) {
    const semantic = {
      topics: [],
      sentiments: [],
      intentions: [],
